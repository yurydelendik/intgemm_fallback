 ...
 *                                  Size of the array = `width` * `cols_B`.
 */
void __attribute__((export_name("int8_prepare_b")))
int8PrepareB(const float* input_B,
             ...
             Index cols_B,
             int8_t* output) {
    ::intgemm::SSSE3::Kernels8::PrepareB(input_B,
                            output,
                            scale,
                            width, cols_B);                 
}
 ...
 *                                     Size of the array = `width` * `cols_B`.
 */
void __attribute__((export_name("int8_prepare_b_from_transposed")))
    int8PrepareBFromTransposed(const float* input_B_transposed,
                               ...
                               Index cols_B,
                               int8_t* output) {
    ::intgemm::SSSE3::Kernels8::PrepareBTransposed(input_B_transposed,
                                      output,
                                      scale,
                                      width, cols_B);                                   
}
 ...
 *                                         Size of the array = `width` * `cols_B`.
 */
void __attribute__((export_name("int8_prepare_b_from_quantized_transposed")))
int8PrepareBFromQuantizedTransposed(const int8_t* input_B_quant_transposed,
                                    Index width,
                                    Index cols_B,
                                    int8_t* output) {
    ::intgemm::SSSE3::Kernels8::PrepareBQuantizedTransposed(
        input_B_quant_transposed, output, width, cols_B);
}
 ...
 *                             Size of the array = `rows_A` * `width`.
 */
void __attribute__((export_name("int8_prepare_a")))
int8PrepareA(const float* input_A,
             ...
             Index width,
             int8_t* output) {
    ::intgemm::SSSE3::Kernels8::QuantizeU(input_A, reinterpret_cast<uint8_t *>(output),
                                   scale, rows_A * width);
}
 ...
 */
void __attribute__((export_name("int8_prepare_bias")))
 ...
 */
void __attribute__((export_name("int8_multiply_and_add_bias")))
    int8MultiplyAndAddBias(const int8_t* input_A_prepared,
                           ...
                           Index cols_B,
                           float* output) {
    float unquantFactor = unquant_multiplier / (scale_A * scale_B);
    ::intgemm::SSSE3::Kernels8::Multiply8Shift(
      (const uint8_t *)input_A_prepared,
      input_B_prepared, rows_A, width, cols_B,
      ::intgemm::callbacks::UnquantizeAndAddBiasAndWrite(
          unquantFactor, input_bias_prepared, output));
}
 ...
 */
void __attribute__((export_name("int8_select_columns_of_b")))
int8SelectColumnsOfB(const int8_t* input_B_prepared,
                     ...
                     int8_t* output) {
    ::intgemm::SSSE3::Kernels8::SelectColumnsB(
        input_B_prepared, output, width, cols, cols + num_cols);
}